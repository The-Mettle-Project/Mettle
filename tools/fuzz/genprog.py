...
class Gen:
    ...
    def gen_structs(self, k):
        ...
        for _ in range(k):
            ...
            mk = [f"{deco}function {make}(x: int64) -> {name} {{",
                  f"  var s: {name};"]
            for fn_, ft in fields:
            ...
            fl = [f"{deco2}function {fold}(s: {name}) -> int64 {{",
    ...
    def gen_globals(self, k):
        for _ in range(k):
            ...
            if self.rng.random() < 0.7:
                reader = f"pr_{gname}"
                self.helpers.append(
                    f"@pure @noinline function {reader}(x: int64) -> int64 {{\n"
    ...
    def gen_helpers(self, k):
        for _ in range(k):
            ...
            body = [f"{deco}function {name}({params}) -> int64 {{"]
    ...
    def gen_float_helpers(self):
        ...
        if self.rng.random() < 0.8:
            ...
            c = self.rng.randint(0, 9)
            self.helpers.append(
                f"function {name}(x: float32, y: float32) -> float64 {{\n"
        ...
        if self.rng.random() < 0.8:
            ...
            c = self.rng.randint(0, 9)
            self.helpers.append(
                f"function {name}(x: float32) -> float32 {{\n"
                f"  return x * {k}.0 + {c}.0;\n"
                f"}}")
        ...
        if self.rng.random() < 0.8:
            ...
            c = self.rng.randint(0, 9)
            self.helpers.append(
                f"function {name}(x: float64) -> int64 {{\n"
    ...
    def gen_kernels(self):
        ...
        if r.random() < 0.85:  # float sum
            ...
            self.helpers.append(
                f"function {name}(a: {ty}*, n: int64) -> {ty} {{\n"
                f"  var s: {ty} = 0.0;\n"
        ...
        if r.random() < 0.85:  # float dot
            ...
            ty = r.choice(["float32", "float64"])
            self.helpers.append(
                f"function {name}(a: {ty}*, b: {ty}*, n: int64) -> {ty} {{\n"
                f"  var s: {ty} = 0.0;\n"
        ...
        if r.random() < 0.8:   # int32 sum (cast-free widening or cast)
            ...
            self.helpers.append(
                f"function {name}(a: int32*, n: int64) -> int64 {{\n"
                f"  var s: int64 = 0;\n"
        ...
        if r.random() < 0.7:   # int32 dot (int64 acc is the dot_i32 shape)
            name = self.fresh("kid")
            self.helpers.append(
                f"function {name}(a: int32*, b: int32*, n: int64) -> int64 {{\n"
                f"  var s: int64 = 0;\n"
                + self.kernel_loop(["s = s + a[i] * b[i];"]) + "\n"
                f"  return s;\n}}")
        ...
        if r.random() < 0.7:   # byte sum
            name = self.fresh("kbs")
            self.helpers.append(
                f"function {name}(a: uint8*, n: int64) -> int64 {{\n"
                f"  var s: int64 = 0;\n"
        ...
        if r.random() < 0.7:   # byte in-place map chain
            ...
            self.helpers.append(
                f"function {name}(a: uint8*, n: int64) {{\n"
                + self.kernel_loop(ops) + "\n}")
            self.kernels["bmap"] = name
        if r.random() < 0.7:   # byte dot -> int32
            name = self.fresh("kbd")
            self.helpers.append(
                f"function {name}(a: uint8*, b: uint8*, n: int64) -> int32 {{\n"
                f"  var s: int32 = 0;\n"
        ...
        if r.random() < 0.5:   # int32 matmul (SLP MAC / MIR-SIMD shape)
            name = self.fresh("kmm")
            self.helpers.append(
                f"function {name}(a: int32*, b: int32*, c: int32*,"
                f" n: int64) {{\n"
                f"  var i: int64 = 0;\n"
                f"  while (i < n) {{\n"
                f"    var j: int64 = 0;\n"
