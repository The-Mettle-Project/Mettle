...
| Kernel scalar/pointer/record ABI | compile-time checked, device-validated | scalars, pointers, and records built from those; a record parameter is passed by value as its own bytes and `--emit-kernel-decls` carries its declaration; closure/string/function-pointer fields are rejected at the source; narrow scalar PTX parameters use natural widths |
...
- Done for records: locals, fixed arrays, runtime indexing, address-of,
  by-value parameters and results, whole-record assignment, and record kernel
  parameters, on PTX with device numerical validation. The OpenCL 2.0 SPIR-V
  profile carries record storage and rejects the by-value call ABI explicitly.
- Extend the direct device-call foundation with generic specialization,
  cross-module device linking, and inlining/cost modeling without weakening the
  portable call-graph rules.
