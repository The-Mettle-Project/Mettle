...
**Limitations:** (1) A closure stored in a struct field cannot yet be *called* through `obj.field(args)` (that syntax resolves as a method call); storing the closure works. (2) A non-capturing lambda / `&func` cannot be passed where an `Fn(...)` closure is expected (no thin-to-closure adaptation yet); use a matching `fn(...)` parameter for plain functions. (3) The captured variables must have an explicit type (closures cannot capture a variable whose type was inferred). (4) Captured-variable mutations inside the closure are call-local and do not persist across calls. (5) The heap environment is not freed automatically (consistent with Mettle's manual-heap model).
