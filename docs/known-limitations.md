...
A non-capturing lambda or `&func` can be passed anywhere an `Fn(...)` closure is expected - as a call argument to a plain function, a variable declaration, a return value, or an assignment to a variable, a struct field, an array element or a field reached through a pointer - and the compiler transparently wraps it in a generated adapter so it dispatches through the closure calling convention. **Limitations:** (1) Adaptation covers a *literal* `&func` or lambda at the boundary; a thin value already sitting in a variable (`var g: fn(...)->R = &func; use(g);`) is not yet adapted - write `use(g)` as `use(&func)` directly, or re-spell `g`'s declared type as `Fn(...)`. (2) The captured variables must have an explicit type (closures cannot capture a variable whose type was inferred). (3) The heap environment (of both closures and adapters) is not freed automatically (consistent with Mettle's manual-heap model).
...
- **`typeof(T).fields` is the only compile-time sequence.** A `comptime for` iterates a struct's fields and nothing else. There is no way to declare a table of data and generate from it.
- **Compile-time strings compare and nothing else.** `==` and `!=` fold, which is enough to check that two declarations agree by name. There is no concatenation, ordering, length, or substring, so a name cannot be computed from parts; `ident(...)` composes declaration names under rules the compiler checks.
