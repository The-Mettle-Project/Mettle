...
static int flat_entry_section(const BinaryEmitter *emitter,
                              const char *entry_symbol, size_t *entry_section,
                              char *error, size_t error_size) {
  const BinarySymbol *entry;
  *entry_section = (size_t)-1;
  if (!entry_symbol || !entry_symbol[0]) {
    return 1;
  }
  entry = flat_find_symbol(emitter, entry_symbol);
  if (!entry) {
...
static size_t flat_place_sections(const BinaryEmitter *emitter,
                                  size_t entry_section,
                                  FlatPlacement *placements,
...
static void flat_write_value(unsigned char *image, size_t site_offset,
                             uint64_t value, size_t bytes) {
  size_t i;
  for (i = 0; i < bytes; i++) {
    image[site_offset + i] = (unsigned char)((value >> (8 * i)) & 0xFFu);
  }
}

static int flat_apply_relocation(unsigned char *image, size_t site_offset,
  ...
  case BINARY_RELOCATION_REL32:
    displacement = (int64_t)target - (int64_t)(site + 4) + relocation->addend;
    ...
    flat_write_value(image, site_offset, (uint64_t)displacement, 4);
    return 1;
  case BINARY_RELOCATION_ADDR64:
    flat_write_value(image, site_offset, value, 8);
    return 1;
  case BINARY_RELOCATION_REL16:
    displacement = (int64_t)target - (int64_t)(site + 2) + relocation->addend;
    ...
    flat_write_value(image, site_offset, (uint64_t)displacement, 2);
    return 1;
  case BINARY_RELOCATION_ADDR16:
    ...
    flat_write_value(image, site_offset, value, 2);
    return 1;
  case BINARY_RELOCATION_ADDR32NB:
    flat_write_value(image, site_offset, value, 4);
    return 1;
  default:
    flat_fail(error, error_size,
...
static int flat_placement_offset(const FlatPlacement *placements,
                                 size_t placement_count, size_t section_index,
                                 size_t *file_offset) {
  size_t i;
  for (i = 0; i < placement_count; i++) {
    if (placements[i].section_index == section_index) {
      *file_offset = placements[i].file_offset;
      return 1;
    }
  }
  return 0;
...
static int flat_apply_relocations(const BinaryEmitter *emitter,
                                  unsigned char *image, uint64_t image_base,
                                  const FlatPlacement *placements,
                                  size_t placement_count, char *error,
                                  size_t error_size) {
  size_t i;
  for (i = 0; i < emitter->relocation_count; i++) {
    ...
    if (!flat_placement_offset(placements, placement_count,
                               relocation->section_index, &placed)) {
      continue;
    }
    site_offset = placed + relocation->offset;
    ...
    if (!flat_apply_relocation(image, site_offset, site,
                               image_base + placed + symbol->value, relocation,
                               error, error_size)) {
      return 0;
    }
  }
...
static int flat_write_file(const char *path, const unsigned char *image,
                           size_t image_size, char *error, size_t error_size) {
...
int binary_emitter_write_flat(BinaryEmitter *emitter, const char *path,
  ...
  if (!flat_entry_section(emitter, entry_symbol, &entry_section, error,
                          error_size)) {
    return 0;
  }
  ...
  cursor = flat_place_sections(emitter, entry_section, placements,
                               &placement_count);
  ...
  if (trailer && trailer_size) {
    ...
    } else if (pad_to < trailer_size || cursor > pad_to - trailer_size) {
      flat_fail(error, error_size,
  ...
  if (!flat_apply_relocations(emitter, image, image_base, placements,
                              placement_count, error, error_size)) {
    goto cleanup;
  }
  ...
  if (!flat_write_file(path, image, image_size, error, error_size)) {
    goto cleanup;
  }
  result = 1;
