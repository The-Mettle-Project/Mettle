...
- **Compile-time memory diagnostics**: use-after-free and double free warnings, leak detection for allocations that never escape their function, and hard errors for returning the address of a stack local, constant indexes past an array's end, and constant-size memory ops that overwrite a stack frame. The analysis only speaks when it is certain; anything conditional stays silent.
