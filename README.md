...
- **Compile-time memory diagnostics**: use-after-free, double free, and leak detection, with hard errors for returning the address of a stack local, constant indexes past an array's end, and constant-size memory ops that overwrite a stack frame. The analysis is interprocedural: it infers per-function ownership (which parameters a function frees, which it keeps, whether it returns a fresh allocation) across the whole program, so `consume(p); p[0]` is caught as use-after-free even when the `free` lives inside `consume`, and a leak is still seen through a helper that only borrows the pointer. It speaks only when certain; anything conditional stays silent.
